Feature: Java Multi Platform Binary Distribution
	...
	Scenario: RISC-V platform is supported and a specific version of compatible binary is installed
		Given an "riscv64" machine with "Linux" installed
		And the system is bootstrapped
		And the candidate "java" version "8.0.111" is available for download on "Linux" with architecture "riscv64"
		And the appropriate multi-platform hook is available for "java" version "8.0.111" on "Linux" with architecture "riscv64"
		When I enter "sdk install java 8.0.111"
		And I see "Done installing!"
		And the candidate "java" version "8.0.111" is installed

	Scenario: RISC-V platform is supported and a default version of compatible binary is installed
		Given an "riscv64" machine with "Linux" installed
		And the system is bootstrapped
		And the default "java" version is "8.0.111"
		And the candidate "java" version "8.0.111" is available for download on "Linux" with architecture "riscv64"
	...
	Scenario: RISC-V platform is supported but download fails
		Given an "riscv64" machine with "Linux" installed
		And the system is bootstrapped
		And the candidate "java" version "8.0.101" is available for download on "Linux" with architecture "riscv64"
		And the appropriate multi-platform hook is available for "java" version "8.0.101" on "Linux" with architecture "riscv64"
		When I enter "sdk install java 8.0.101"
		And I see "Download has failed, aborting!"
		And the candidate "java" version "8.0.101" is not installed
		And I see "Cannot install java 8.0.101 at this time..."

	Scenario: RISC-V unsupported version on platform and user is notified
		And an "riscv64" machine with "Linux" installed
		And the system is bootstrapped
