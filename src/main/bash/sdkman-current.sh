#!/usr/bin/env bash

#
#   Copyright 2017 Marco Vermeulen
#
#   Licensed under the Apache License, Version 2.0 (the "License");
#   you may not use this file except in compliance with the License.
#   You may obtain a copy of the License at
#
#       http://www.apache.org/licenses/LICENSE-2.0
#
#   Unless required by applicable law or agreed to in writing, software
#   distributed under the License is distributed on an "AS IS" BASIS,
#   WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
#   See the License for the specific language governing permissions and
#   limitations under the License.
#

function __sdk_current {
	__sdkman_validate_non_blank_argument_counts "sdk ${COMMAND}" 0 1 'candidate' "${@}" || return 1

	local candidate="${1}"

	if [ -n "${candidate}" ]; then
		__sdkman_validate_candidate "${candidate}" || return 1
		__sdkman_determine_current_version "${candidate}"
		if [ -n "${CURRENT}" ]; then
			__sdkman_echo_no_colour "\nUsing ${candidate} version ${CURRENT}"
		else
			__sdkman_echo_red "\nNot using any version of ${candidate}"
		fi
	else
		local installed_count=0
		for candidate in ${SDKMAN_CANDIDATES[@]}; do
			# Eliminate empty entries due to incompatibility
			if [[ -n "${candidate}" ]]; then
				__sdkman_determine_current_version "${candidate}"
				if [ -n "${CURRENT}" ]; then
					if [ "${installed_count}" -eq 0 ]; then
						__sdkman_echo_no_colour $'\nUsing:\n'
					fi
					__sdkman_echo_no_colour "${candidate}: ${CURRENT}"
					((installed_count += 1))
				fi
			fi
		done

		if [ "${installed_count}" -eq 0 ]; then
			__sdkman_echo_no_colour $'\nNo candidates are in use'
		fi
	fi
}

function __sdkman_determine_current_version {
	local candidate
	candidate="${1}"

	if __sdkman_path_contains "${SDKMAN_CANDIDATES_DIR}/${candidate}"; then
		case "${platform}" in
		solaris) CURRENT=$(echo "${PATH}" | gsed -r "s|${SDKMAN_CANDIDATES_DIR}/${candidate}/([^/]+)/bin|!!\1!!|1" | gsed -r 's|^.*!!(.+)!!.*$|\1|g') ;;
		darwin)  CURRENT=$(echo "${PATH}" |  sed -E "s|${SDKMAN_CANDIDATES_DIR}/${candidate}/([^/]+)/bin|!!\1!!|1" |  sed -E 's|^.*!!(.+)!!.*$|\1|g') ;;
		*)       CURRENT=$(echo "${PATH}" |  sed -r "s|${SDKMAN_CANDIDATES_DIR}/${candidate}/([^/]+)/bin|!!\1!!|1" |  sed -r 's|^.*!!(.+)!!.*$|\1|g') ;;
		esac

		if [[ "${CURRENT}" == 'current' ]]; then
			CURRENT="$(basename "$(readlink "${SDKMAN_CANDIDATES_DIR}/${candidate}/current")")"
		fi
	else
		CURRENT=
	fi
}
