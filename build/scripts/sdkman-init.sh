...
export SDKMAN_VERSION="1.0.0-SNAPSHOT"
export SDKMAN_PLATFORM=$(uname)

if [ -z "${SDKMAN_SERVICE}" ]; then
    export SDKMAN_SERVICE="http://dev.sdkman.io"
fi

if [ -z "${SDKMAN_BROADCAST_SERVICE}" ]; then
    export SDKMAN_BROADCAST_SERVICE="http://cast-dev.sdkman.io"
fi

if [ -z "${SDKMAN_BROKER_SERVICE}" ]; then
    export SDKMAN_BROKER_SERVICE="http://broker-dev.sdkman.io"
fi
